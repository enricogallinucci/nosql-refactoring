...
CREATE TABLE PhotoObjAll_Galaxy AS (
	...
	FROM (
		...
		FROM aa_SDSS2013_10x_optimized.PhotoObjAll_Galaxy as g 
		  JOIN aa_SDSS2013_10x_optimized.PhotoObjAll_GalaxyComplementary gc ON g.key=gc.KEY
		UNION ALL
		-- Galaxies that are primary objects
		SELECT *
		FROM (
					SELECT pp.KEY, pp.value||pc.value AS value
					FROM aa_SDSS2013_10x_optimized.PhotoObjAll_Primary pp  
						JOIN aa_SDSS2013_10x_optimized.PhotoObjAll_PrimaryComplementary pc ON pp.KEY=pc.KEY --({objidlist})
		...
		UNION ALL
		SELECT *
		FROM aa_SDSS2013_10x_optimized.PhotoObjAll_Other
...
-- This takes all PhotoObjAll_Other in 2013 optimized
	...
	FROM aa_SDSS2013_10x_optimized.PhotoObjAll_Other p
...
CREATE TABLE SpecObjAll AS (
	...
	FROM aa_SDSS2013_10x_optimized.SpecObjAll s
);
...
CREATE TABLE Photoz AS (
	...
	FROM aa_SDSS2013_10x_optimized.Photoz p
);
...
FROM aa_SDSS2013_10x_optimized.Frame fr 
  JOIN aa_SDSS2013_10x_optimized.Field fi ON fi.key=fr.key1
WHERE fr.key2=0);
...
CREATE TABLE Frame_Other AS (
	SELECT *  
	FROM aa_SDSS2013_10x_optimized.Frame f
WHERE f.key2<>0
);
...
-- 	SELECT key, value 
-- 	FROM aa_SDSS2013_10x_optimized.Platex p
-- );
...
CREATE TABLE Field_Other AS (
	SELECT * 
	FROM aa_SDSS2013_10x_optimized.Field fi
	WHERE NOT EXISTS (SELECT 'Found' FROM aa_SDSS2013_10x_optimized.Frame fr WHERE fi.key=fr.key1)
);
...
-- 	SELECT * 
-- 	FROM aa_SDSS2013_10x_optimized.GalSpecExtra g
-- );
...
-- 	SELECT * 
-- 	FROM aa_SDSS2013_10x_optimized.GalSpecIndx g
-- );
