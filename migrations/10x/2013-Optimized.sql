...
FROM aa_SDSS2003_10x_optimized.PhotoObjAll_Galaxy as g 
	JOIN aa_SDSS2003_10x_optimized.PhotozRF pzr ON pzr.key=g.KEY
	JOIN aa_SDSS2003_10x_optimized.PhotoObjAll_GalaxyComplementary gc ON gc.key=g.key
...
FROM (
			...
			FROM (SELECT * FROM aa_SDSS2003_10x_optimized.PhotoObjAll_Primary) pp 
				JOIN (SELECT * FROM aa_SDSS2003_10x_optimized.PhotoObjAll_PrimaryComplementary) pc ON pp.KEY=pc.KEY
	...
	JOIN aa_SDSS2003_10x_optimized.Photoz as p ON g.key=p.key
	JOIN aa_SDSS2003_10x_optimized.PhotozRF pzr ON pzr.key=g.KEY
WHERE ((g.value->>'flags')::int8 & 262144) = 0
);
...
FROM aa_SDSS2003_10x_optimized.PhotoObjAll_Galaxy as g 
WHERE ((g.value->>'flags')::int8 & 262144) = 0
	AND EXISTS (SELECT 'Found' FROM aa_SDSS2003_10x_optimized.Photoz as p WHERE g.key=p.key)
	AND EXISTS (SELECT 'Found' FROM aa_SDSS2003_10x_optimized.PhotozRF as pzr WHERE g.key=pzr.key)
UNION ALL
-- Galaxies that are primary objects in 2003 optimized
...
FROM aa_SDSS2003_10x_optimized.PhotoObjAll_Primary p
  JOIN aa_SDSS2003_10x_optimized.PhotoObjAll_PrimaryComplementary pc ON p.key=pc.KEY
WHERE (p.value->>'type')::int8 <> 3 --OR (pc.value->>'type')::int4<>3 --class='GALAXY' (see https://skyserver.sdss.org/dr1/en/help/browser/browser.asp)
  OR ((pc.value->>'flags')::int8 & 262144) <> 0
	OR NOT EXISTS (SELECT 'Found' FROM aa_SDSS2003_10x_optimized.Photoz as pz WHERE p.key=pz.key)
	OR NOT EXISTS (SELECT 'Found' FROM aa_SDSS2003_10x_optimized.PhotozRF as pzr WHERE p.key=pzr.key)
);
...
FROM aa_SDSS2003_10x_optimized.PhotoObjAll_Primary pp
  JOIN aa_SDSS2003_10x_optimized.PhotoObjAll_PrimaryComplementary pc ON pp.key=pc.KEY
WHERE (pp.value->>'type')::int8 <> 3 --OR (pc.value->>'type')::int4<>3 --class='GALAXY' (see https://skyserver.sdss.org/dr1/en/help/browser/browser.asp)
  OR ((pc.value->>'flags')::int8 & 262144) <> 0
	OR NOT EXISTS (SELECT 'Found' FROM aa_SDSS2003_10x_optimized.Photoz as p WHERE pp.key=p.key)
	OR NOT EXISTS (SELECT 'Found' FROM aa_SDSS2003_10x_optimized.PhotozRF as pzr WHERE pp.key=pzr.key)
);
...
FROM aa_SDSS2003_10x_optimized.PhotoObjAll_Other p
UNION ALL
...
FROM aa_SDSS2003_10x_optimized.PhotoObjAll_Galaxy g
  JOIN aa_SDSS2003_10x_optimized.PhotoObjAll_GalaxyComplementary gc ON g.key=gc.KEY
WHERE (((gc.value->>'flags')::int8 & 262144) <> 0
	OR NOT EXISTS (SELECT 'Found' FROM aa_SDSS2003_10x_optimized.Photoz as p WHERE g.key=p.key)
	OR NOT EXISTS (SELECT 'Found' FROM aa_SDSS2003_10x_optimized.PhotozRF as pzr WHERE g.key=pzr.key))
);
...
CREATE TABLE SpecObjAll AS (
	SELECT *
	FROM aa_SDSS2003_10x_optimized.SpecObjAll s
	UNION ALL
	SELECT c.KEY, c.value||(g.value->'SpecObj')::jsonb
	FROM aa_SDSS2003_10x_optimized.SpecObjAll_Complementary c
	  JOIN aa_SDSS2003_10x_optimized.PhotoObjAll_Galaxy g ON g.key=(c.value->>'bestobjid')::int8
);
...
CREATE TABLE Photoz AS (
	...
	FROM (
	  SELECT *
	  FROM aa_SDSS2003_10x_optimized.Photoz p
	  UNION ALL
		SELECT c.key, c.value||(g.value->'Photoz')::jsonb AS value
		FROM aa_SDSS2003_10x_optimized.Photoz_Complementary c
			JOIN aa_SDSS2003_10x_optimized.PhotoObjAll_Galaxy g ON g.key=c.key
		) p
...
CREATE TABLE PhotozRF AS (
	SELECT *
	FROM aa_SDSS2003_10x_optimized.PhotozRF p
	WHERE NOT EXISTS (SELECT 'Found' FROM PhotoObjAll_Galaxy g WHERE g.key=p.key )
);
...
-- 	SELECT * 
-- 	FROM aa_SDSS2003_10x_optimized.Field f
-- );
...
-- 	SELECT *  
-- 	FROM aa_SDSS2003_10x_optimized.Frame f
-- );
...
-- 	SELECT * 
-- 	FROM aa_SDSS2003_10x_optimized.GalSpecIndx g
-- );
