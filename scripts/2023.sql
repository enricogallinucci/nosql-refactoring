--========================================================= 2003 Original ===============================================================================
DROP SCHEMA IF EXISTS aa_SDSS2023 CASCADE;
CREATE SCHEMA aa_SDSS2023;
SET search_path TO aa_SDSS2023, public;
SHOW search_path;
...
EXPLAIN (ANALYZE TRUE, COSTS FALSE, SUMMARY true)
SELECT g.value->>'ra', g.value->>'dec', g.value->>'dered_r', s.value->>'z', p.value->>'z'
FROM PhotoObjAll g 
	LEFT OUTER JOIN SpecObjAll s ON g.KEY=(s.value->>'bestobjid')::int8
	LEFT OUTER JOIN Photoz as p ON g.key=p.KEY
WHERE (g.value->>'type')::int8=3 --AND (pc.value->>'type')::int8=3 --class='GALAXY' (see https://skyserver.sdss.org/dr1/en/help/browser/browser.asp)
	AND g.key IN (1237645941824356443) --({objidlist})
	AND (p.value->>'z')::float8 > 0 AND (p.value->>'z')::float8 < 1 -- pz1.z < {z2} AND pz1.z > {z1}
	AND (g.value->>'dered_r')::float8 > 0 AND (g.value->>'dered_r')::float8 < 100 -- p.dered_r < {dered_r2} AND p.dered_r > {dered_r1}
;
...
EXPLAIN (ANALYZE TRUE, COSTS FALSE, SUMMARY true)
SELECT DISTINCT p.value->>'ra', p.value->>'dec', p.key, p.value->>'run', p.value->>'rerun', p.value->>'camcol', p.value->>'field', s.value->>'z', s.value->>'plate', s.value->>'mjd', s.value->>'fiberid', s.value->>'specobjid', s.value->>'run2d'
FROM PhotoObjAll p 
...
FROM PhotoObjAll p 
	JOIN SpecObjAll s ON p.key=(s.value->>'bestobjid')::int8
WHERE (s.value->>'plate')::int8=422 		-- s.plate={plate}
	AND (s.value->>'mjd')::int8=51811 		-- s.mjd={mjd}
	AND (s.value->>'fiberid')::int8=390; -- s.fiberid={fiberid}
...
FROM PhotoObjAll p 
	JOIN SpecObjAll s ON p.key=(s.value->>'bestobjid')::int8
  JOIN Platex px ON px.key=(s.value->>'plateid')::float8
WHERE (s.value->>'scienceprimary')::int8=1
	AND ((s.value->>'ra')::float8 BETWEEN 15 AND 20) --((p.ra between {ra1} and {ra2}) and
	AND ((s.value->>'dec')::float8 BETWEEN 15 AND 20)  -- (p.dec between {dec1} and {dec2}))
;
...
EXPLAIN (ANALYZE TRUE, COSTS FALSE, SUMMARY true)
SELECT p.key, p.value->>'psffwhm_r', p.value->>'field', p.value->>'run', p.value->>'camcol', p.value->>'rowc_r', p.value->>'colc_r', p.value->>'rowc', p.value->>'colc', p.value->>'fracdev_r', p.value->>'devab_r', p.value->>'devphi_r', p.value->>'insidemask', p.value->>'flags', p.value->>'sky_r', p.value->>'petroflux_r', p.value->>'petrofluxivar_r', p.value->>'fiber2flux_r', p.value->>'petrorad_r', p.value->>'petroraderr_r', p.value->>'petror50_r', p.value->>'petror50err_r', p.value->>'petror90_r', p.value->>'petror90err_r', p.value->>'devrad_r', p.value->>'devraderr_r', p.value->>'devflux_r', p.value->>'devfluxivar_r', p.value->>'airmass_r', p.value->>'cloudcam_r', p.value->>'calibstatus_r', p.value->>'expflux_r', p.value->>'expab_r', p.value->>'exprad_r', p.value->>'expphi_r', p.value->>'psfflux_r'   
				, px.value->>'seeing50'
				, s.value->>'z', s.value->>'zerr', s.value->>'zwarning', s.value->>'class', s.value->>'z_noqso', s.value->>'zerr_noqso', s.value->>'zwarning_noqso', s.value->>'veldisp', s.value->>'veldisperr', s.value->>'veldispz', s.value->>'veldispzerr', s.value->>'veldispchi2', s.value->>'veldispnpix', s.value->>'veldispdof', s.value->>'snmedian_r', s.value->>'snmedian', s.value->>'chi68p', s.value->>'fracnsigma_1', s.value->>'fracnsighi_1', s.value->>'fracnsiglo_1', s.value->>'spectroflux_r', s.value->>'spectrosynflux_r', s.value->>'spectrofluxivar_r', s.value->>'spectrosynfluxivar_r', s.value->>'instrument', s.value->>'bossspecobjid', s.value->>'specobjid', s.value->>'bestobjid', s.value->>'plate', s.value->>'fiberid' 
FROM PhotoObjAll p 
	JOIN SpecObjAll s ON p.key=(s.value->>'bestobjid')::int8
  JOIN Platex px ON px.key=(s.value->>'plateid')::float8
WHERE (s.value->>'scienceprimary')::int8=1
