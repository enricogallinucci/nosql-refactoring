/* REPLACE:
sdss_relational2 with sdss_relational_2x
aa_SDSS2003_2x with aa_SDSS2003_2x
aa_SDSS2013_2x with aa_SDSS2013_2x
aa_SDSS2023_2x with aa_SDSS2023_2x
(the three above include the optimized versions)
*/

--========================================================= 2003 Original ===============================================================================
DROP SCHEMA IF EXISTS aa_SDSS2003_2x CASCADE;
CREATE SCHEMA aa_SDSS2003_2x;
SET search_path TO aa_SDSS2003_2x, public;
SHOW search_path;
...
CREATE TABLE PhotoObjAll AS (
  ...
  FROM sdss_relational_2x.PhotoObjAll AS p);
...
CREATE TABLE Photoz AS (
	SELECT p.objid as key,
    		(to_jsonb(p.*) - 'objid') AS value
	FROM sdss_relational_2x.photoz p);
...
CREATE TABLE SpecObjAll AS (
	...
	FROM sdss_relational_2x.SpecObjAll AS s);
...
--========================================================= 2003 Optimized ===============================================================================
DROP SCHEMA IF EXISTS aa_SDSS2003_2x_optimized CASCADE;
CREATE SCHEMA aa_SDSS2003_2x_optimized;
SET search_path TO aa_SDSS2003_2x_optimized, public;
SHOW search_path;
...
CREATE TABLE PhotoObjAll_Primary AS (
	...
	FROM aa_SDSS2003_2x.PhotoObjAll p 
...
CREATE TABLE PhotoObjAll_Galaxy AS (
	...
	FROM aa_SDSS2003_2x.PhotoObjAll as g 
		LEFT OUTER JOIN aa_SDSS2003_2x.SpecObjAll s ON g.key=(s.value->>'bestobjid')::int8
		JOIN aa_SDSS2003_2x.Photoz as p ON g.key=p.key
...
CREATE TABLE PhotoObjAll_GalaxyComplementary AS (
	...
	WHERE (g.value->>'type')::int8=3 --class='GALAXY' (see https://skyserver.sdss.org/dr1/en/help/browser/browser.asp) 
		AND (g.value->>'mode')::int8<>1
		AND EXISTS (SELECT 'Found' FROM aa_SDSS2003_2x.Photoz as p WHERE g.key=p.key)
);
	...
	WHERE (p.value->>'mode')::int8<>1
	  AND ((p.value->>'type')::int8<>3 --class<>'GALAXY' (see https://skyserver.sdss.org/dr1/en/help/browser/browser.asp)
	    OR ((p.value->>'type')::int8=3 --class='GALAXY' (see https://skyserver.sdss.org/dr1/en/help/browser/browser.asp) 
				AND NOT EXISTS (SELECT 'Found' FROM aa_SDSS2003_2x.Photoz as pz WHERE pz.key=p.key)))
);
...
CREATE TABLE SpecObjAll AS (
	SELECT *
	FROM aa_SDSS2003_2x.SpecObjAll s
...
CREATE TABLE PhotozRF AS (
	SELECT p.objid as key,
    			(to_jsonb(p.*) - 'objid') AS value 
	FROM sdss_relational_2x.PhotozRF p
);
...
CREATE TABLE Field AS (
	...
	FROM sdss_relational_2x.Field f
);
...
CREATE TABLE Frame AS (
	...
	FROM sdss_relational_2x.Frame f
);
...
CREATE TABLE GalSpecIndx AS (
	...
	FROM sdss_relational_2x.GalSpecIndx g
);
...
CREATE TABLE Platex AS (
	...
	FROM sdss_relational_2x.Platex p
);
...
CREATE TABLE GalSpecExtra AS (
	SELECT g.specobjid as key,
    			(to_jsonb(g.*) - 'specobjid') AS value 
	FROM sdss_relational_2x.GalSpecExtra g
);
...
--========================================================= 2013 Optimized ===============================================================================
DROP SCHEMA IF EXISTS aa_SDSS2013_2x_optimized CASCADE;
CREATE SCHEMA aa_SDSS2013_2x_optimized;
SET search_path TO aa_SDSS2013_2x_optimized, public;
SHOW search_path;
...
FROM aa_SDSS2003_2x_optimized.PhotoObjAll_Galaxy as g 
	JOIN aa_SDSS2003_2x_optimized.PhotozRF pzr ON pzr.key=g.KEY
	JOIN aa_SDSS2003_2x_optimized.PhotoObjAll_GalaxyComplementary gc ON gc.key=g.key
...
FROM (
			...
			FROM (SELECT * FROM aa_SDSS2003_2x_optimized.PhotoObjAll_Primary) pp 
				JOIN (SELECT * FROM aa_SDSS2003_2x_optimized.PhotoObjAll_PrimaryComplementary) pc ON pp.KEY=pc.KEY
	...
	JOIN aa_SDSS2003_2x_optimized.Photoz as p ON g.key=p.key
	JOIN aa_SDSS2003_2x_optimized.PhotozRF pzr ON pzr.key=g.KEY
WHERE ((g.value->>'flags')::int8 & 262144) = 0
);
ALTER TABLE PhotoObjAll_Galaxy ADD PRIMARY KEY (key);
...
FROM aa_SDSS2003_2x_optimized.PhotoObjAll_Galaxy as g 
WHERE ((g.value->>'flags')::int8 & 262144) = 0
	AND EXISTS (SELECT 'Found' FROM aa_SDSS2003_2x_optimized.Photoz as p WHERE g.key=p.key)
	AND EXISTS (SELECT 'Found' FROM aa_SDSS2003_2x_optimized.PhotozRF as pzr WHERE g.key=pzr.key)
UNION ALL
-- Galaxies that are primary objects in 2003 optimized
...
FROM aa_SDSS2003_2x_optimized.PhotoObjAll_Primary p
  JOIN aa_SDSS2003_2x_optimized.PhotoObjAll_PrimaryComplementary pc ON p.key=pc.KEY
WHERE (p.value->>'type')::int8 <> 3 --OR (pc.value->>'type')::int4<>3 --class='GALAXY' (see https://skyserver.sdss.org/dr1/en/help/browser/browser.asp)
  OR ((pc.value->>'flags')::int8 & 262144) <> 0
	OR NOT EXISTS (SELECT 'Found' FROM aa_SDSS2003_2x_optimized.Photoz as pz WHERE p.key=pz.key)
	OR NOT EXISTS (SELECT 'Found' FROM aa_SDSS2003_2x_optimized.PhotozRF as pzr WHERE p.key=pzr.key)
);
ALTER TABLE PhotoObjAll_Primary ADD PRIMARY KEY (key);
ANALYZE PhotoObjAll_Primary;
...
FROM aa_SDSS2003_2x_optimized.PhotoObjAll_Primary pp
  JOIN aa_SDSS2003_2x_optimized.PhotoObjAll_PrimaryComplementary pc ON pp.key=pc.KEY
WHERE (pp.value->>'type')::int8 <> 3 --OR (pc.value->>'type')::int4<>3 --class='GALAXY' (see https://skyserver.sdss.org/dr1/en/help/browser/browser.asp)
  OR ((pc.value->>'flags')::int8 & 262144) <> 0
	OR NOT EXISTS (SELECT 'Found' FROM aa_SDSS2003_2x_optimized.Photoz as p WHERE pp.key=p.key)
	OR NOT EXISTS (SELECT 'Found' FROM aa_SDSS2003_2x_optimized.PhotozRF as pzr WHERE pp.key=pzr.key)
);
ALTER TABLE PhotoObjAll_PrimaryComplementary ADD PRIMARY KEY (key);
ANALYZE PhotoObjAll_PrimaryComplementary;
...
FROM aa_SDSS2003_2x_optimized.PhotoObjAll_Other p
UNION ALL
...
FROM aa_SDSS2003_2x_optimized.PhotoObjAll_Galaxy g
  JOIN aa_SDSS2003_2x_optimized.PhotoObjAll_GalaxyComplementary gc ON g.key=gc.KEY
WHERE (((gc.value->>'flags')::int8 & 262144) <> 0
	OR NOT EXISTS (SELECT 'Found' FROM aa_SDSS2003_2x_optimized.Photoz as p WHERE g.key=p.key)
	OR NOT EXISTS (SELECT 'Found' FROM aa_SDSS2003_2x_optimized.PhotozRF as pzr WHERE g.key=pzr.key))
);
ALTER TABLE PhotoObjAll_Other ADD PRIMARY KEY (key);
ANALYZE PhotoObjAll_Other;
...
CREATE TABLE SpecObjAll AS (
	SELECT *
	FROM aa_SDSS2003_2x_optimized.SpecObjAll s
	UNION ALL
	SELECT c.KEY, c.value||(g.value->'SpecObj')::jsonb
	FROM aa_SDSS2003_2x_optimized.SpecObjAll_Complementary c
	  JOIN aa_SDSS2003_2x_optimized.PhotoObjAll_Galaxy g ON g.key=(c.value->>'bestobjid')::int8
);
ALTER TABLE SpecObjAll ADD PRIMARY KEY (key);
ANALYZE SpecObjAll;
...
CREATE TABLE Photoz AS (
	...
	FROM (
	  SELECT *
	  FROM aa_SDSS2003_2x_optimized.Photoz p
	  UNION ALL
		SELECT c.key, c.value||(g.value->'Photoz')::jsonb AS value
		FROM aa_SDSS2003_2x_optimized.Photoz_Complementary c
			JOIN aa_SDSS2003_2x_optimized.PhotoObjAll_Galaxy g ON g.key=c.key
		) p
	WHERE NOT EXISTS (SELECT 'Found' FROM PhotoObjAll_Galaxy g WHERE g.key=p.key )
);
ALTER TABLE Photoz ADD PRIMARY KEY (key);
...
CREATE TABLE PhotozRF AS (
	SELECT *
	FROM aa_SDSS2003_2x_optimized.PhotozRF p
	WHERE NOT EXISTS (SELECT 'Found' FROM PhotoObjAll_Galaxy g WHERE g.key=p.key )
);
ALTER TABLE PhotozRF ADD PRIMARY KEY (key);
ANALYZE PhotozRF;
...
CREATE TABLE Field AS (
	SELECT * 
	FROM aa_SDSS2003_2x_optimized.Field f
);
ALTER TABLE Field ADD PRIMARY KEY (key);
ANALYZE Field;
...
CREATE TABLE Frame AS (
	SELECT *  
	FROM aa_SDSS2003_2x_optimized.Frame f
);
ALTER TABLE Frame ADD PRIMARY KEY (key1, key2);
ANALYZE Frame;
...
CREATE TABLE GalSpecIndx AS (
	SELECT * 
	FROM aa_SDSS2003_2x_optimized.GalSpecIndx g
);
ALTER TABLE GalSpecIndx ADD PRIMARY KEY (key);
ANALYZE GalSpecIndx;
...
--========================================================= 2023 Optimized ===============================================================================
DROP SCHEMA IF EXISTS aa_SDSS2023_2x_optimized CASCADE;
CREATE SCHEMA aa_SDSS2023_2x_optimized;
SET search_path TO aa_SDSS2023_2x_optimized, public;
SHOW search_path;
...
CREATE TABLE PhotoObjAll_Galaxy AS (
	...
	FROM (
		...
		FROM aa_SDSS2013_2x_optimized.PhotoObjAll_Galaxy as g 
		  JOIN aa_SDSS2013_2x_optimized.PhotoObjAll_GalaxyComplementary gc ON g.key=gc.KEY
		UNION ALL
		-- Galaxies that are primary objects
		SELECT *
		FROM (
					SELECT pp.KEY, pp.value||pc.value AS value
					FROM aa_SDSS2013_2x_optimized.PhotoObjAll_Primary pp  
						JOIN aa_SDSS2013_2x_optimized.PhotoObjAll_PrimaryComplementary pc ON pp.KEY=pc.KEY --({objidlist})
		...
		UNION ALL
		SELECT *
		FROM aa_SDSS2013_2x_optimized.PhotoObjAll_Other
...
-- This takes all PhotoObjAll_Other in 2013 optimized
	SELECT p.key, p.value 
	FROM aa_SDSS2013_2x_optimized.PhotoObjAll_Other p
...
CREATE TABLE SpecObjAll AS (
	...
	FROM aa_SDSS2013_2x_optimized.SpecObjAll s
);
ALTER TABLE SpecObjAll ADD PRIMARY KEY (key);
...
CREATE TABLE Photoz AS (
	...
	FROM aa_SDSS2013_2x_optimized.Photoz p
);
ALTER TABLE Photoz ADD PRIMARY KEY (key);
ANALYZE Photoz;
...
FROM aa_SDSS2013_2x_optimized.Frame fr 
  JOIN aa_SDSS2013_2x_optimized.Field fi ON fi.key=fr.key1
WHERE fr.key2=0);
...
CREATE TABLE Frame_Other AS (
	SELECT *  
	FROM aa_SDSS2013_2x_optimized.Frame f
WHERE f.key2<>0
);
...
CREATE TABLE Platex AS (
	SELECT key, value 
	FROM aa_SDSS2013_2x_optimized.Platex p
);
ALTER TABLE Platex ADD PRIMARY KEY (key);
ANALYZE Platex;
...
CREATE TABLE Field_Other AS (
	SELECT * 
	FROM aa_SDSS2013_2x_optimized.Field fi
	WHERE NOT EXISTS (SELECT 'Found' FROM aa_SDSS2013_2x_optimized.Frame fr WHERE fi.key=fr.key1)
);
...
CREATE TABLE GalSpecExtra AS (
	SELECT * 
	FROM aa_SDSS2013_2x_optimized.GalSpecExtra g
);
ALTER TABLE GalSpecExtra ADD PRIMARY KEY (key);
ANALYZE GalSpecExtra;
...
CREATE TABLE GalSpecIndx AS (
	SELECT * 
	FROM aa_SDSS2013_2x_optimized.GalSpecIndx g
);
ALTER TABLE GalSpecIndx ADD PRIMARY KEY (key);
ANALYZE GalSpecIndx;