...
FROM PhotoObjAll p 
WHERE (p.value->>'mode')::int8=1 AND p.key IN (1237645941824356443) --({objidlist})
LIMIT 1;
...
WHERE g.key IN (1237645941824356443) --({objidlist})
	AND (g.value->>'type')::int8=3 --class='GALAXY' (see https://skyserver.sdss.org/dr1/en/help/browser/browser.asp)
	AND (g.value->>'i')::float8 BETWEEN 14.0 AND 21.0 -- Should be BETWEEN 15 AND 21
	AND (p.value->>'z')::float8 > 0; --{z1}	
...
WHERE g.key IN (1237645941824356443) --({objidlist}) 
	AND (g.value->>'type')::int8=3 --class='GALAXY' (see https://skyserver.sdss.org/dr1/en/help/browser/browser.asp)
	AND (g.value->>'i')::float8 BETWEEN 14.0 AND 21.0 -- Should be BETWEEN 15 AND 21
	AND (p.value->>'z')::float8 BETWEEN 0 AND 1; --{z1} and {z2}
