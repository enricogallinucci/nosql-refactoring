--========================================================= 2023 Optimized ===============================================================================
DROP SCHEMA IF EXISTS aa_SDSS2023_optimized CASCADE;
CREATE SCHEMA aa_SDSS2023_optimized;
SET search_path TO aa_SDSS2023_optimized, public;
SHOW search_path;
...
EXPLAIN (ANALYZE TRUE, COSTS FALSE, SUMMARY true)
SELECT g.value->>'ra' AS ra, g.value->>'dec' AS dec, g.value->>'dered_r', s.value->>'z' AS sz, pz.value->>'z' AS pz
...
WHERE g.key IN (1237645941824356443) --({objidlist})
	AND (pz.value->>'z')::float8 > 0 AND (pz.value->>'z')::float8 < 1 -- pz1.z < {z2} AND pz1.z > {z1}
...
WHERE p.key=1237661064950973129; -- {objid}
...
FROM ( 
	...
	FROM (
			SELECT * 
			FROM PhotoObjAll_Primary p 
			WHERE ((p.value->>'ra')::float8 BETWEEN 134.89276313971027 and 188.31470893856442)  
				AND ((p.value->>'dec')::float8 BETWEEN 20.06564507544314 and 48.4028703158333) 
			) p 
	JOIN PhotoObjAll_PrimaryComplementary pc ON p.KEY=pc.key 
	UNION ALL 
	SELECT g.KEY, g.value||gc.value AS value 
